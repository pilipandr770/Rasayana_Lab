...
(function () {
  ...
  style.textContent = `
    ...
    #rasaChatHead{background:var(--bg-hero); color:var(--on-hero); padding:10px 14px; display:flex; justify-content:space-between; align-items:flex-start; font-size:14.5px; font-weight:700;}
    #rasaChatHead button{background:none; border:none; color:var(--on-hero); font-size:18px; cursor:pointer; line-height:1;}
    #rasaChatSubtitle{display:block; font-size:11px; font-weight:400; color:var(--on-hero-soft); margin-top:2px;}
  ...
  btn.id = "rasaChatBtn";
  btn.setAttribute("aria-label", "AI chat assistant");
  btn.textContent = "🤖";

  const panel = document.createElement("div");
  panel.id = "rasaChatPanel";
  panel.innerHTML = `
    <div id="rasaChatHead"><span><span id="rasaChatTitle"></span><span id="rasaChatSubtitle"></span></span><button id="rasaChatClose" type="button">✕</button></div>
  ...
  const subtitleEl = panel.querySelector("#rasaChatSubtitle");

  let history = [];

  function applyLang() {
    titleEl.textContent = tr("Запитайте про проєкт", "Ask about the project", "Fragen Sie zum Projekt");
    subtitleEl.textContent = tr("🤖 ІІ-асистент, не людина", "🤖 AI assistant, not a human", "🤖 KI-Assistent, kein Mensch");
    ...
    if (history.length === 0) {
      msgsEl.innerHTML = "";
      addMsg("bot", tr(
        "Привіт! Я ІІ-асистент проєкту Rasayana (не людина) і можу пояснити, як влаштований проєкт і токен — питайте.",
        "Hi! I'm Rasayana's AI assistant (not a human) and can explain how the project and token work — ask away.",
        "Hallo! Ich bin der KI-Assistent von Rasayana (kein Mensch) und kann erklären, wie das Projekt und der Token funktionieren — fragen Sie einfach."
      ));
    }
  }
